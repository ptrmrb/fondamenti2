...
{	private NodoInt testa;
	...
	{
		return VERIFICA(k , testa, 0);
	}

	private boolean VERIFICA ( int k , NodoInt n, int count)
	{
		if ( n == null ) return true;
		
		if ( count < k ){
			if ( n.getInfo() < k )
				return VERIFICA(k, n.getSuccessivo(), count + 1 );
			return false;
		}
		else
			if ( n.getInfo() > k )
				return VERIFICA(k, n.getSuccessivo(), count );
			return false; 
	}	

	public static void main(String[] args) {
		ListaConcatenataInt lista = new ListaConcatenataInt(new int[] {1,0,2,6,4});
		Terminale.stampa(lista.VERIFICA(3));
	}
}
