...
public class ListaConcatenataInt1 
{	private NodoInt testa;
	private NodoInt coda;
	private int lunghezza;
	
	public ListaConcatenataInt1()
	{	inizializza();
	}
	
	public ListaConcatenataInt1(ListaConcatenataInt1 l)
	{	inizializza();
	...
	public ListaConcatenataInt1(int[] a)
	{	inizializza();
		for(int v : a)
			aggiungiInCoda(v);
	}
	
	public ListaConcatenataInt1(ArrayList<Integer> a)
	{	inizializza();
		for(int v : a)
			aggiungiInCoda(v);
	}
	
	public ListaConcatenataInt1(LinkedList<Integer> a)
	{	inizializza();
		for(int v : a)
			aggiungiInCoda(v);
	}
	...
	{	if(o == null)
		...
		if(o == this)
			return true;
		if(!(o instanceof ListaConcatenataInt1))
			return false;
		ListaConcatenataInt1 l = (ListaConcatenataInt1)o;
	...
	public ListaConcatenataInt1 listaInvertita()
	{	ListaConcatenataInt1 ret = new ListaConcatenataInt1();
		for(NodoInt corrente = testa; corrente != null; 
				corrente = corrente.getSuccessivo())
	...
	/*
	Si arricchisca la classe ListaConcatenataInt sviluppata durante il corso
	con un esaminaLista() che controlla che ogni elemento della lista sia maggiore
	o uguale alla somma di tutti i successivi,  ad esempio se la lista è
	[50, 22, 10, 5, 2, 1] allora la funzione restituisce True.

	Il metodo dovrà essere ricorsivo o invocare un opportuno metodo ricorsivo sulla
	classe NodoInt.
	*/

	public boolean esaminaLista()
	{
		return esaminaLista(testa);
	}

	private boolean esaminaLista(NodoInt n)
	{
		if ( n == null ) return true;

		if ( n.getInfo() >= sommaDa(n.getSuccessivo()) )
			return esaminaLista(n.getSuccessivo());
		return false; 
	}
	...
	*/
	...
	int[] lista = {50, 22, 10, 5, 2, 1};
	ListaConcatenataInt1 l = new ListaConcatenataInt1(lista);

	Terminale.stampa(l.esaminaLista());
	}
