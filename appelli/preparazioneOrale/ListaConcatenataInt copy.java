...
{	private NodoInt testa;
	...
	/*
	 * Si arricchisca la classe ListaConcatenataInt sviluppata durante il corso con
	 * un metodo verificaBudget() che riceve in input tre double b1, b2, b3 e un’altra
	 * ListaConcatenataInt l1, i tre double indicano i budget dei clienti 1, 2 e 3 mentre
	 * il generico elemento della lista l1 indica chi ha effettuato la spesa che è presente
	 * nella lista originale,  ad esempio se la lista è [20, 26, 10, 62, 91, 41, 12, 36, 6, 18],
	 * b1= 78, b2=123, b3=121 e l1=[ 2, 2, 1, 1, 3, 2, 3, 2, 1, 3] allora la funzione restituisce True.
	 * 
	 * Il metodo dovrà essere ricorsivo o invocare un opportuno metodo ricorsivo sulla classe NodoInt.
	 * 
	 */

	public boolean verificaBudget( double b1 , double b2 , double b3 , ListaConcatenataInt l1)
	{
		return verificaBudgetRicorsivo(testa, l1.testa, b1, b2, b3);
	}

	private boolean verificaBudgetRicorsivo(NodoInt n, NodoInt m,  double b1 , double b2 , double b3 )  
	// n è il nodo della lista dei valori mentre m è il nodo della lista dei clienti																						
	{																												
		if ( n == null || m == null ) return 0 == b1 && 0 == b2 && 0 == b3;

		if ( m.getInfo() == 1)
			return verificaBudgetRicorsivo( n.getSuccessivo(), m.getSuccessivo(), b1 -= n.getInfo(), b2, b3);

		else if ( m.getInfo() == 2 ) 
			return verificaBudgetRicorsivo( n.getSuccessivo(), m.getSuccessivo(), b1, b2-= n.getInfo(), b3);
		return verificaBudgetRicorsivo( n.getSuccessivo(), m.getSuccessivo(), b1, b2, b3-= n.getInfo());
	}

	public static void main(String[] args) {
		
		int[] listaPrezzi = {20, 26, 10, 62, 91, 41, 12, 36, 6, 18};
		int[] listaClienti = { 2, 2, 1, 1, 3, 2, 3, 2, 1, 3 };

		ListaConcatenataInt lP = new ListaConcatenataInt(listaPrezzi);
		ListaConcatenataInt lC = new ListaConcatenataInt(listaClienti);

		Terminale.stampa(lP.verificaBudget(78, 123, 121, lC));
	}
